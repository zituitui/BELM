...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        if use_apex_fuse:
            from .apex_models import FusedLinear
            Linear_cls = FusedLinear
        else:
        ...
        self.to_q = Linear_cls(query_dim, inner_dim, bias=bias)

        if not self.only_cross_attention:
            # only relevant for the `AddedKVProcessor` classes
            self.to_k = Linear_cls(cross_attention_dim, inner_dim, bias=bias)
            self.to_v = Linear_cls(cross_attention_dim, inner_dim, bias=bias)
        else:
            self.to_k = None
            self.to_v = None

        if self.added_kv_proj_dim is not None:
            self.add_k_proj = Linear_cls(added_kv_proj_dim, inner_dim)
            self.add_v_proj = Linear_cls(added_kv_proj_dim, inner_dim)

        self.to_out = nn.ModuleList([])
        self.to_out.append(Linear_cls(inner_dim, query_dim, bias=out_bias))
